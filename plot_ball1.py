...
def getV():
    """Takes user input for a set of v_0's, returns them as array, for
    use later in getTY()"""
    v = np.array([float(i) for i in raw_input("Enter your list of v_0's, separated by commas: ").split(",")])
    return v
    
def getTY(v):
    """Takes the array v as a parameter, and creates arrays for t and 
    the resulting f(t,v). So for each i, t[i], y[i] will be an initial value,
    an array of 100 times, and an array of 100 data points (y values).
    This function returns a list l = [t, y]."""
